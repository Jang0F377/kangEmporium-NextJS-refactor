...
interface RenderCommentListProps {
  comment: Comment;
}

const CommentComponent = ({ comments, _id }: CommentsProps) => {
  const [allowWriteReview, setAllowWriteReview] = useState(false);
  const reviews = {
    average: 4,
    totalCount: 1624,
    counts: [
  ...
  const handleAllowReview = () => {
    setAllowWriteReview(!allowWriteReview);
  };
      ...
      <div className="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:py-32 lg:px-8 lg:grid lg:grid-cols-12 lg:gap-x-8">
        <div className="lg:col-span-4">
          ...
          <div className="mt-10">
            ...
            >
              {allowWriteReview ? "Close Review" : "Write a Review"}
            </button>
            {allowWriteReview ? <WriteAReview _id={_id} /> : null}
          </div>
        </div>
...
const ReviewListItemComponent = ({ comment }: RenderCommentListProps) => {
  return (
    <div key={comment._id} className="py-12">
      <div className="flex items-center">
        <div className="ml-4">
